...
def get_random_input(num_inputs):
    input_tensors = [torch.randint(0, num_inputs, (num_inputs, ), dtype=torch.int32).cuda() for i in range(4)]
    warmup_input = input_tensors[0]
    stable_sort_input = input_tensors[1]
    stable_sort_opaque_input = input_tensors[2]
    torch_sort_input = input_tensors[3]
    return warmup_input, stable_sort_input, stable_sort_opaque_input, torch_sort_input
...
def torch_sort(input_tensor):
    values, indices = torch.sort(input_tensor, dim=0)
    return values, indices
...
def profile_torch_sort(torch_sort_input):
    with profile(activities=[
            ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True, with_stack=True) as prof:
        torch_sort(torch_sort_input)
        torch.cuda.synchronize()

    for event in prof.key_averages():
        if "DeviceRadixSortOnesweep" in event.key:
            torch_sort_time = event.self_device_time_total / 4
            break
    print(f"DeviceRadixSortOnesweepKernel latency in torch.sort is {torch_sort_time:.2f} us")

if __name__ == "__main__":
    enforce_reproduce()
    
    num_inputs = 100000000
    warmup_input, stable_sort_input, stable_sort_opaque_input, torch_sort_input = get_random_input(num_inputs)

    profile_stable_sort(stable_sort_input)
    profile_stable_sort_opaque(stable_sort_opaque_input)
    profile_torch_sort(torch_sort_input)
